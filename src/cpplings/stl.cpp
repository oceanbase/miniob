...
int main() {
    ...
    std::transform(algVec.begin(), algVec.end(), algVec3.begin(), 
                  [](int n) { return n * 2; });
    ...
    // 计数
    int count = std::count_if(algVec3.begin(), algVec3.end(), 
                             [](int n) { return n > 10; });
    std::cout << "Number of elements > 10 in algVec3: " << count << std::endl;
    
    // 所有元素满足条件？
    bool all = std::all_of(algVec.begin(), algVec.end(), 
                         [](int n) { return n < 100; });
    std::cout << "Are all elements in algVec < 100? " << (all ? "Yes" : "No") << std::endl;
    
    // 任意元素满足条件？
    bool any = std::any_of(algVec.begin(), algVec.end(), 
                         [](int n) { return n > 8; });
    ...
    // 内积
    int product = std::inner_product(algVec.begin(), algVec.end(), 
                                   algVec3.begin(), 0);
