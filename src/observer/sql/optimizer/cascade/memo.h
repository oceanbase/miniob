...
class Memo {
 public:
  ...
  void record_operator(unique_ptr<OperatorNode>&& node) {
    operator_nodes_.emplace(node.get(), std::move(node));
  }
 ...
 private:
  ...
  struct GExprPtrEq {
    bool operator()(GroupExpr *const &t1, GroupExpr *const &t2) const { return (*t1 == *t2); }
  };
  ...
  // TODO: 这是用来存储在 optimize 过程中生成的临时物理算子节点的，有些物理算子节点的所有权会转移到外面，有些物理算子的所有权还在memo，需要删除。
  // 用 shared_ptr 更加合适，但是改动比较大，先暂时不改了。
