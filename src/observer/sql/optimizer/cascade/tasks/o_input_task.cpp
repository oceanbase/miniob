...
void OptimizeInputs::perform()
{
  ...
  if (cur_child_idx_ == -1) {
    ...
    cur_total_cost_ += context_->get_cost_model()->calculate_cost(&context_->get_memo(), group_expr_);
  }
  for (; cur_child_idx_ < static_cast<int>(group_expr_->get_children_groups_size()); cur_child_idx_++) {
    auto child_group = context_->get_memo().get_group_by_id(group_expr_->get_child_group_id(cur_child_idx_));
    ...
    if (child_best_expr != nullptr) {
      ...
      if (cur_total_cost_ > context_->get_cost_upper_bound())
        break;
