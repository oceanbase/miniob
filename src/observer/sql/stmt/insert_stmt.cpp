...
InsertStmt::InsertStmt(Table *table, const Inserts &inserts)
  : table_ (table)
{
  item_num_ = inserts.item_num;
  for(int i = 0; i < item_num_; i++) {
    value_amount_[i] = inserts.value_num[i];
    values_[i] = inserts.values[i];
  }
}
...
RC InsertStmt::create(Db *db, const Inserts &inserts, Stmt *&stmt)
{
  ...
  for(int i = 0; i < inserts.item_num; i++) {
    if(inserts.value_num[i] <= 0) {
      LOG_WARN("invalid argument. value_num\n");
      return RC::INVALID_ARGUMENT;
    }
  }
  ...
  size_t item_num = inserts.item_num;
