...
RC MvccTrx::update_record(Table *table, Record &old_record, Record &new_record)
{
  ...
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to delete old record while updating under mvcc. rc=%s", strrc(rc));
    return rc;
  }

  Record inserted_record = new_record;
  rc                     = insert_record(table, inserted_record);
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to insert new record while updating under mvcc. rc=%s", strrc(rc));
    return rc;
  }
