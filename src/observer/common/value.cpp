...
void Value::set_data(char *data, int length)
{
  switch (attr_type_) {
    case AttrType::CHARS:
    case AttrType::TEXTS: {
      set_string(data, length, attr_type_);
    } break;
    case AttrType::INTS: {
...
void Value::set_string(const char *s, int len /*= 0*/, AttrType type /*= AttrType::CHARS*/)
{
  reset();
  attr_type_ = type;
  if (s == nullptr) {
...
void Value::set_value(const Value &value)
{
  switch (value.attr_type_) {
    ...
    case AttrType::CHARS:
    case AttrType::TEXTS: {
      set_string(value.get_string().c_str(), value.length(), value.attr_type_);
    } break;
    case AttrType::BOOLEANS: {
...
{
  ASSERT(attr_type_ == AttrType::CHARS || attr_type_ == AttrType::TEXTS, "attr type is not a string type");
